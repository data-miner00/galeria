<script lang="ts">
	...
	let filteredImages = $derived(
		activeCategory === 'All'
			? results.filter((image) => !image.isSoftDeleted)
			: results.filter((image) => !image.isSoftDeleted && image.category === activeCategory)
	);
	...
	{:else}
		<Empty.Root>
			<Empty.Header>
				...
				<Empty.Description
					>{searchQuery ? `No results found for "${searchQuery}"` : 'Please enter a search query.'} Try
					adjusting your search query.</Empty.Description
				>
			</Empty.Header>
			<Empty.Content>
				<div class="flex gap-2">
